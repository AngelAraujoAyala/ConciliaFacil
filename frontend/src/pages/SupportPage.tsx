...
export function SupportPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Encabezado */}
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Centro de Soporte</h1>
        <p className="text-slate-500">¿Tienes dudas o encontraste un problema? Estamos aquí para ayudarte a resolverlo.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {faqs.map((faq, index) => {
              ...
              return (
                <div key={index} className="border-b border-slate-100 last:border-none">
                  ...
                  >
                    <span className="font-medium text-slate-700 text-sm sm:text-base">{faq.question}</span>
                    {isOpen ? <ChevronUp size={18} className="text-slate-400" /> : <ChevronDown size={18} className="text-slate-400" />}
                  </button>
                  {isOpen && (
                    <div className="px-5 pb-4 text-sm text-slate-600 bg-slate-50/30">
                      {faq.answer}
                    </div>
                  )}
                </div>
        ...
        <div className="space-y-6">
          <h2 className="text-lg font-semibold text-slate-700">Contacto Directo</h2>
          
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
            <p className="text-xs text-slate-400 font-semibold tracking-wider uppercase">Vías de atención</p>
            
            {/* Contenedor relativo para posicionar el modal pegado a este botón */}
            <div className="relative">
              <button 
                onClick={(e) => handleContactClick(e, 'mail')}
                className="w-full flex items-center gap-4 p-3 rounded-lg border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all text-left group"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">soporte@conciliafacil.com</p>
                </div>
              </button>
              ...
              {activeModal === 'mail' && (
                <div 
                  ref={modalRef}
                  className="absolute right-0 bottom-full mb-2 lg:right-full lg:bottom-auto lg:top-0 lg:mr-2 z-50 w-72 bg-slate-900 text-white p-4 rounded-xl shadow-xl border border-slate-800 text-xs sm:text-sm animate-in fade-in slide-in-from-bottom-2 duration-200"
                >
            ...
            <div className="relative">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-slate-700">Soporte por WhatsApp</p>
